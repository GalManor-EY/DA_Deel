/* ============================================================
   ...
   Q1-2026
   Logic:
   TMS Q1 Withdrawal transactions whose reconciliation_id
   does NOT appear in PAYOUT Q1
   ============================================================ */

WITH all_bank_transactions_filtered_q1_2026 AS (
    SELECT *
    FROM dbo.stg_tms_transactions_q1_2026
    WHERE TRY_CONVERT(date, LEFT(created_at, 10)) >= '2026-01-01'
      AND TRY_CONVERT(date, LEFT(created_at, 10)) <  '2026-04-01'
...
tms_only_contractor_withdrawals AS (
    SELECT
        ...
        atw.usd_amount,
        apcw.WITHDRAWAL_ID
    FROM all_bank_transactions_filtered_q1_2026 atw
...
payout_q1_2026 AS (
    ...
    WHERE TRY_CONVERT(date, LEFT(withdrawal_created_at, 10)) >= '2026-01-01'
      AND TRY_CONVERT(date, LEFT(withdrawal_created_at, 10)) <  '2026-04-01'
),

tms_not_in_payout_q1_2026 AS (
    SELECT *
...
FROM tms_not_in_payout_q1_2026 t
...
GROUP BY
    ...
    END,
    apcw.STATUS
ORDER BY payout_from_year, payout_from_month, exists_in_full_payout;